...
public class RefactoringDiscoveryResults {
    ...
    public void populateJSON(String repository, String sha1, String url, List<org.refactoringminer.api.Refactoring> refactoring1, List<org.remapper.diff.Refactoring> refactoring2) {
        Result result = new Result(repository, sha1, url);
        for (org.refactoringminer.api.Refactoring refactoring : refactoring1) {
            Refactoring refactor1 = new Refactoring(refactoring);
            result.addBaseline(refactor1);
        }
        for (org.remapper.diff.Refactoring refactoring : refactoring2) {
            Refactoring refactor2 = new Refactoring(refactoring);
            result.addOurApproach(refactor2);
        }
        results.add(result);
    }
...
//        private boolean developerConfirmation;
        ...
        public Refactoring(org.remapper.diff.Refactoring refactoring) {
            this.type = refactoring.getRefactoringType().toString();
            this.description = refactoring.toString();
