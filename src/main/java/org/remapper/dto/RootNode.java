...
public class RootNode extends DeclarationNodeTree {
    ...
    public List<DeclarationNodeTree> getAllNodes() {
        if (allNodes == null) {
            ...
            depthFirstSearch(getChildren(), allNodes);
        }
        return allNodes;
    }
    ...
    private void depthFirstSearch(List<DeclarationNodeTree> children, List<DeclarationNodeTree> list) {
        for (DeclarationNodeTree child : children) {
            depthFirstSearch(child.getChildren(), list);
            list.add(child);
        }
    }
}
