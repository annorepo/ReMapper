...
public class MatchServiceImpl implements MatchService {
    ...
    public MatchServiceImpl(String projectPath) {
    ...
    public void matchAtCommit(Repository repository, String commitId, MatchingHandler handler) {
        ...
        try {
            ...
            if (commit.getParentCount() > 0) {
                ...
                this.matchEntities(repository, handler, commit);
            }
        } catch (Exception e) {
    ...
    private void matchEntities(Repository repository, final MatchingHandler handler, RevCommit currentCommit) throws Exception {
        ...
        populateCurrentDependencies(matchPair, modifiedFiles, renamedFiles, addedFiles);
        gitService.resetHard(repository);
        gitService.checkoutParent(repository, commitId);
        populateBeforeDependencies(matchPair, modifiedFiles, renamedFiles, deletedFiles);
        gitService.resetHard(repository);
    ...
    private boolean pruneUnchangedEntities(MatchPair matchPair, DeclarationNodeTree dntBefore, DeclarationNodeTree dntCurrent) {
        if (StringUtils.compare(dntBefore.getDeclaration().toString(), dntCurrent.getDeclaration().toString())) {
//        if (dntBefore.getHashCode() == dntCurrent.getHashCode()) {
            if (dntBefore.isRoot() && dntCurrent.isRoot() && dntBefore.hasChild() && dntCurrent.hasChild())
                pruneUnchangedEntities(matchPair, dntBefore.getChildren(), dntCurrent.getChildren());
            return true;
        } else if (dntBefore.hasChild() && dntCurrent.hasChild())
            pruneUnchangedEntities(matchPair, dntBefore.getChildren(), dntCurrent.getChildren());
        return false;
    }

    private void pruneUnchangedEntities(MatchPair matchPair, List<DeclarationNodeTree> dntBefore, List<DeclarationNodeTree> dntCurrent) {
        List<DeclarationNodeTree> deletionBefore = new ArrayList<>();
        List<DeclarationNodeTree> deletionCurrent = new ArrayList<>();
        for (DeclarationNodeTree node1 : dntBefore) {
            for (DeclarationNodeTree node2 : dntCurrent) {
        ...
        pruneEntities(dntBefore, dntCurrent, deletionBefore, deletionCurrent);
    }
...
//        if (dntBefore.getHashCode() == dntCurrent.getHashCode()) {
    ...
    private void pruneUnchangedEntities(MatchPair matchPair, String filePath, String renamedFilePath, List<DeclarationNodeTree> dntBefore, List<DeclarationNodeTree> dntCurrent) {
        List<DeclarationNodeTree> deletionBefore = new ArrayList<>();
        List<DeclarationNodeTree> deletionCurrent = new ArrayList<>();
        for (DeclarationNodeTree node1 : dntBefore) {
            for (DeclarationNodeTree node2 : dntCurrent) {
    ...
    private void addInternalCandidateEntity(MatchPair matchPair, String filePath, String renamedFilePath, DeclarationNodeTree node1, DeclarationNodeTree node2) {
        if (node1.equals(node2, filePath, renamedFilePath) &&
                StringUtils.compare(node1.getDeclaration().toString(), node2.getDeclaration().toString())) {
            matchPair.addMatchedEntity(node1, node2);
            if (node1.hasChild() && node2.hasChild()) {
    ...
    private void pruneEntities(List<DeclarationNodeTree> dntBefore, List<DeclarationNodeTree> dntCurrent, List<DeclarationNodeTree> deletionBefore, List<DeclarationNodeTree> deletionCurrent) {
        DeclarationNodeTree parentBefore = dntBefore.get(0).getParent();
        ...
        DeclarationNodeTree parentCurrent = dntCurrent.get(0).getParent();
    ...
    private void matchByNameAndSignature(MatchPair matchPair, Set<String> modifiedFiles, Map<String, RootNode> fileDNTsBefore, Map<String, RootNode> fileDNTsCurrent) {
        for (String filePath : modifiedFiles) {
            RootNode dntBefore = fileDNTsBefore.get(filePath);
            RootNode dntCurrent = fileDNTsCurrent.get(filePath);
            if (!dntBefore.hasChild() || !dntCurrent.hasChild())
                continue;
            ...
            for (DeclarationNodeTree node1 : treeNodesBefore) {
                for (DeclarationNodeTree node2 : treeNodesCurrent) {
                    ...
                    if (node1.getType() == EntityType.CLASS || node1.getType() == EntityType.INTERFACE ||
                        ...
                        if (StringUtils.compare(fd1.getType().toString(), fd2.getType().toString())) {
                            node1.setMatched();
                            node2.setMatched();
                            matchPair.addMatchedEntity(node1, node2);
                            break;
                        ...
                        String tp2 = ((List<TypeParameter>) md2.typeParameters()).stream().
                                map(TypeParameter::toString).
                                collect(Collectors.joining(","));
                        if (md1.getReturnType2() == null && md2.getReturnType2() == null && StringUtils.compare(pl1, pl2) &&
                                StringUtils.compare(tp1, tp2)) {
                            node1.setMatched();
                            node2.setMatched();
                            matchPair.addMatchedEntity(node1, node2);
                            break;
                        ...
                        if (md1.getReturnType2() != null && md2.getReturnType2() != null &&
                                StringUtils.compare(md1.getReturnType2().toString(), md2.getReturnType2().toString()) &&
                                StringUtils.compare(pl1, pl2) && StringUtils.compare(tp1, tp2)) {
                            node1.setMatched();
                            node2.setMatched();
                            matchPair.addMatchedEntity(node1, node2);
                            break;
                        ...
                        if (StringUtils.compare(atd1.getType().toString(), atd2.getType().toString())) {
                            node1.setMatched();
                            node2.setMatched();
                            matchPair.addMatchedEntity(node1, node2);
                            break;
        ...
        for (InternalNode internalBefore : internalNodesBefore) {
            for (InternalNode internalCurrent : internalNodesCurrent) {
                if (internalBefore.getType() != internalCurrent.getType())
                    continue;
                double dice = DiceFunction.calculateDice(internalBefore, internalCurrent, matchPair);
                if (dice < DiceFunction.minDice)
                    continue;
    ...
    private void populateEntityDependencies(ProjectParser parser, Map<EntityInfo, List<EntityInfo>> dependencies) throws IOException {
        for (String filePath : parser.getRelatedJavaFiles()) {
            ...
            List<AnnotationTypeMemberDeclaration> typeMemberDeclarations = visitor.getTypeMemberDeclarations();
            ...
            populateDependencyInTypeMemberDeclaration(typeMemberDeclarations, dependencies, cu, filePath);
        ...
        for (FieldDeclaration declaration : fieldDeclarations) {
            ...
            for (VariableDeclarationFragment fragment : fragments) {
                ...
                entityUsages.removeIf(dependency -> dependency.getType() == EntityType.FIELD &&
                        StringUtils.compare(dependency.getName(), variableBinding.getName()) &&
                        StringUtils.compare(dependency.getContainer(), declaringClass.getQualifiedName()));
    ...
    private void populateDependencyInMethodDeclaration(List<MethodDeclaration> methodDeclarations, Map<EntityInfo, List<EntityInfo>> dependencies, CompilationUnit cu, String filePath) {
        for (MethodDeclaration declaration : methodDeclarations) {
            ...
            entityUsages.removeIf(dependency -> dependency.getType() == EntityType.METHOD &&
                    StringUtils.compare(dependency.getSignature(), methodBinding.toString()) &&
                    StringUtils.compare(dependency.getContainer(), declaringClass.getQualifiedName()));
    ...
    private void populateDependencyInTypeMemberDeclaration(List<AnnotationTypeMemberDeclaration> typeMemberDeclarations, Map<EntityInfo, List<EntityInfo>> dependencies, CompilationUnit cu, String filePath) {
        for (AnnotationTypeMemberDeclaration declaration : typeMemberDeclarations) {
            NodeUsageVisitor visitor = new NodeUsageVisitor();
            declaration.accept(visitor);
            List<EntityInfo> entityUsages = visitor.getEntityUsages();
    ...
    private void populateCurrentDependencies(MatchPair matchPair, Set<String> modifiedFiles, Map<String, String> renamedFiles,
    ...
    private void populateBeforeDependencies(MatchPair matchPair, Set<String> modifiedFiles, Map<String, String> renamedFiles,
            ...
            for (DeclarationNodeTree dntBefore : beforeEntities) {
                for (DeclarationNodeTree dntCurrent : currentEntities) {
                    if (dntBefore.getType() == dntCurrent.getType() ||
                        ...
                        double dice = DiceFunction.calculateSimilarity(dntBefore, dntCurrent, matchPair);
                        if (dice < DiceFunction.minDice)
                            continue;
            ...
            for (DeclarationNodeTree dntCurrent : matchPair.getAddedEntities()) {
                if (dntBefore.equals(dntCurrent)) {
                    ...
                    if (dntBefore instanceof InternalNode && dntCurrent instanceof InternalNode) {
                        dice = DiceFunction.calculateDice((InternalNode) dntBefore, (InternalNode) dntCurrent, matchPair);
